...
# ---------------------------------------------------------------------------
# New tools: Binders, Designed Molecules, Protein Structures,
#            Platform Growth, Drug Candidate Comparison
# ---------------------------------------------------------------------------


@mcp.tool()
async def get_designed_binders(
    target: Optional[str] = None,
) -> list[dict[str, Any]]:
    """Fetch designed protein binders from the SMA NIM binder catalogue.

    Returns de novo protein binders that have been designed for SMA targets
    using RFdiffusion + ProteinMPNN, stored in the platform after successful
    design runs.  Binders can be filtered by target symbol.

    Args:
        target: Optional target symbol to filter by (e.g. "NCALD", "SMN2",
                "ROCK2", "UBA1").  Leave empty to return all stored binders.

    Returns:
        List of binder records, each containing: design_id, target_symbol,
        sequence (single-letter amino-acid string), binder_length,
        mpnn_score (0–1, higher = better sequence packing),
        plddt_mean (AlphaFold confidence, 0–100), hotspot_residues,
        pipeline (RFdiffusion + ProteinMPNN steps used), and created_at.
    """
    params: dict[str, Any] = {}
    if target:
        params["target"] = target
    result = await _get("/nims/binders", params=params or None)
    if _is_error(result):
        return [result]  # type: ignore[list-item]
    return result if isinstance(result, list) else result.get("items", result)
...
@mcp.tool()
async def get_designed_molecules(
    target: Optional[str] = None,
    min_qed: Optional[float] = None,
) -> list[dict[str, Any]]:
    """Fetch AI-generated drug-like molecules from the SMA NIM molecule catalogue.

    Returns molecules generated by GenMol NIM and stored in the platform
    after virtual screening runs.  Results can be filtered by target and by
    minimum QED (Quantitative Estimate of Drug-likeness) score.

    Args:
        target: Optional target symbol to filter by (e.g. "SMN2", "CORO1C",
                "STMN2").  Leave empty to return all stored molecules.
        min_qed: Optional minimum QED threshold (0.0–1.0).  QED > 0.6 is
                 considered drug-like; QED > 0.8 is excellent.

    Returns:
        List of molecule records, each containing: molecule_id, target_symbol,
        smiles, qed_score, docking_score, composite_score, bbb_permeable
        (blood-brain barrier permeability prediction), lipinski_pass (bool),
        mw (molecular weight), logp, and created_at.
    """
    params: dict[str, Any] = {}
    if target:
        params["target"] = target
    if min_qed is not None:
        params["min_qed"] = min_qed
    result = await _get("/nims/designed-molecules", params=params or None)
    if _is_error(result):
        return [result]  # type: ignore[list-item]
    return result if isinstance(result, list) else result.get("items", result)
...
async def get_protein_structures(
    ...
    """Fetch protein structures available for SMA drug discovery.

    Returns AlphaFold-predicted and experimentally determined structures
    stored in the platform.  Useful for understanding structural coverage
    across SMA targets and for selecting high-confidence structures for
    downstream docking or binder design.

    Args:
        target: Optional target symbol to filter by (e.g. "CORO1C", "PLS3",
                "UBA1").  Leave empty to return all stored structures.
        min_plddt: Optional minimum mean pLDDT confidence score (0–100).
                   Values >= 70 indicate reliable global fold; >= 90 indicates
                   very high confidence.

    Returns:
        List of structure records, each containing: structure_id,
        target_symbol, source (alphafold / pdb / rfdiffusion), pdb_id,
        alphafold_id, plddt_mean, pae_score, druggable_pockets (int),
        top_pocket_score, structure_url, resolution_angstrom (if
        experimental), confidence_category (high/medium/low), and created_at.
    """
    params: dict[str, Any] = {}
    if target:
        params["target"] = target
    if min_plddt is not None:
        params["min_plddt"] = min_plddt
    result = await _get("/structures", params=params or None)
    if _is_error(result):
        return [result]  # type: ignore[list-item]
    return result if isinstance(result, list) else result.get("items", result)
...
@mcp.tool()
async def get_platform_growth() -> dict[str, Any]:
    """Fetch growth timeline, milestones, and cumulative computational stats.

    Returns a chronological view of the SMA Research Platform's development,
    including current entity counts, key discovery milestones reached,
    and an accounting of the computational resources consumed across all
    NIM-based pipelines (structure prediction, virtual screening, binder
    design, molecule optimisation).

    Returns:
        Dict containing:
          - current_counts: live entity totals (claims, targets, hypotheses,
            drugs, trials, sources, hypotheses)
          - milestones: list of milestone events with date, description, and
            significance
          - computational_resources: cumulative GPU hours, NIM API calls,
            molecules generated, structures predicted, binders designed, and
            docking runs completed
          - growth_timeline: list of weekly or monthly snapshots of entity
            counts showing the growth curve
          - generated_at: timestamp of this snapshot
    """
    result = await _get("/platform/growth")
    if _is_error(result):
        return result
    return result
...
@mcp.tool()
async def compare_drug_candidates(
    drug_names: list[str],
) -> dict[str, Any]:
    """Compare multiple SMA drug candidates head-to-head across key metrics.

    Fetches DiffDock docking scores, QED drug-likeness, mechanism of action,
    approval status, and target coverage for each named drug and assembles a
    structured comparison table.  Useful for quickly ranking candidates before
    committing to experimental validation.

    This tool is ideal for questions like:
    "Which of fasudil, riluzole, and risdiplam is the best starting point for
    a combination therapy?"

    Args:
        drug_names: List of drug names to compare (2–10 drugs).  Use common
                    names or brand names (e.g. ["riluzole", "fasudil",
                    "risdiplam", "nusinersen"]).

    Returns:
        Dict containing:
          - candidates: list of per-drug records, each with drug_name,
            approval_status, mechanism, primary_targets, best_docking_score,
            best_docking_target, qed_score, bbb_permeable, and
            repurposing_score
          - comparison_table: 2-D dict keyed by metric then drug_name for
            easy tabular display
          - recommendation: summary statement identifying the top-ranked
            candidate and key trade-offs
          - generated_at: timestamp
    """
    if not drug_names or len(drug_names) < 2:
        return {
            "error": "Provide at least 2 drug names to compare.",
            "detail": "drug_names must be a list with 2–10 entries.",
        }
    cleaned = [d.strip() for d in drug_names if d.strip()][:10]
    result = await _get(
        "/drugs/compare",
        params={"names": ",".join(cleaned)},
    )
    if _is_error(result):
        return result
    return result
