...
async def generate_modality_hypotheses(target_id: str) -> list[dict]:
    """Generate multiple hypotheses per target, split by claim type clusters.

    Groups claims by therapeutic modality (drug_efficacy, splicing, gene_expression, etc.)
    and generates separate hypotheses for each cluster with ≥2 claims.
    """
    target = await fetchrow("SELECT * FROM targets WHERE id = $1", target_id)
    if not target:
        return []
